 ...
 * 3. 普通上传：失败提示「建议使用大文件上传」、支持选择上传目标空间/文件夹
 * 4. 大文件上传支持选择上传目标空间/文件夹
 * 5. 操作日志：优先从 X-Forwarded-For/X-Real-IP 获取客户端 IP
 * 6. FilePreview 移除调试 console.log
 *
 * 数据库：chunk_uploads 表在服务启动时自动补全 file_id 列（见 server/config/database.js ensureChunkUploadsFileId）。
 * 升级后需重新构建前端并重启服务。
 */
...
async function updateVersion(projectRoot, newVersion) {
  ...
  versionInfo.upgradeHistory.push({
    ...
    description: '从 1.0.7 升级到 1.0.8 - 空间卡片布局、大文件上传与进度修复、上传可选空间/文件夹、操作日志客户端IP、chunk_uploads.file_id、分块上传文件名编码'
  });
...
async function main() {
  ...
  log('  大文件上传与上传体验、操作日志 IP、文件名编码', 'blue');
  log('========================================\n', 'blue');
  ...
  try {
    ...
    log('本次更新内容:', 'yellow');
    log('1. 空间卡片操作区布局优化');
    log('2. 大文件上传：进度与状态修复、可选空间/文件夹、完成上传 file_id 与文件名编码');
    log('3. 普通上传：失败提示大文件上传、可选空间/文件夹');
    log('4. 操作日志：客户端 IP（X-Forwarded-For/X-Real-IP）');
    log('5. 数据库：chunk_uploads 表在首次启动时自动补全 file_id 列');
    log('');
